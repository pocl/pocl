...
#ifndef ONEDNN_UTILS_H
#define ONEDNN_UTILS_H
...
static dnnl_engine_kind_t
validate_engine_kind (dnnl_engine_kind_t akind)
{
...
#define CHECK(f)                                                              \
  do                                                                          \
    {                                                                         \
      dnnl_status_t s_ = f;                                                   \
      if (s_ != dnnl_success)                                                 \
        COMPLAIN_DNNL_ERROR_AND_EXIT (#f, s_);                                \
    }                                                                         \
  while (0)

static inline dnnl_engine_kind_t
parse_engine_kind (int argc, char **argv)
{
...
engine_kind2str_upper (dnnl_engine_kind_t kind)
{
  if (kind == dnnl_cpu)
    return "CPU";
  if (kind == dnnl_gpu)
...
static inline void
read_from_dnnl_memory (void *handle, dnnl_memory_t mem)
{
  dnnl_engine_t eng;
  ...
  if (!handle)
    COMPLAIN_EXAMPLE_ERROR_AND_EXIT ("%s", "handle is NULL.");
  ...
  if (eng_kind == dnnl_gpu || is_cpu_sycl)
    {
      ...
      if (mapped_ptr)
        memcpy (handle, mapped_ptr, bytes);
...
static inline void
write_to_dnnl_memory (const void *handle, dnnl_memory_t mem)
{
  dnnl_engine_t eng;
  dnnl_engine_kind_t eng_kind;
  const dnnl_memory_desc_t *md;
  ...
  if (eng_kind == dnnl_gpu)
    {
      ...
      CHECK (dnnl_memory_map_data (mem, &mapped_ptr));
      if (mapped_ptr)
        memcpy (mapped_ptr, handle, bytes);
      CHECK (dnnl_memory_unmap_data (mem, mapped_ptr));
      return;
    }
#endif
