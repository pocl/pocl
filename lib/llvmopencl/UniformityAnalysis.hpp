//===- UniformityAnalysis.hpp - divergence analysis --*- C++ -*-===//
//
// Adapted from the RV Project, under the Apache License v2.0 with LLVM
// Exceptions. See https://llvm.org/LICENSE.txt for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
// Adaptations: Use local or LLVM SDA depending on the LLVM version
// Remove dependency on VectorMapping and having a dedicated
...
public:
  VectorizationAnalysis(VectorizationInfo &VecInfo, llvm::LoopInfo &LI,
...
private:
  ...
  // add all instruction of \p BB to the WL that dependend on the shape of the
  // predicate. (eg functions with side effects)
  void pushPredicatedInsts(const llvm::BasicBlock &BB);

  // Control divergence propagation (RootNodeType == Loop or
  // Instruction(~Terminator)) \p BranchLoop       the loop containing the
  // divergent node \p UniqueSuccessors uniqued set of immediate successors of
  // this node \p rootNode         the node handle \p domBoundBlock    Immediate
  // dominator of all UniqueSuccessors
  void propagateControlDivergence(
      const llvm::Loop *BranchLoop,
  ...
  // .. for divergent loops
  // propagates control divergence caused by divergent loop exits of \p
  // ExitingLoops.
