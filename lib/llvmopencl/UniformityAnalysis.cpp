...
//
// Main Adaptations: compitability with LLVM 14 (e.g. getNumArgOperands() ->
// args()) Don't use FAM for analyses, but pass them in directly. Small fix and
// cleanups.
//
//===----------------------------------------------------------------------===//
...
VectorizationAnalysis::VectorizationAnalysis(VectorizationInfo &VecInfo,
                                             LoopInfo &LI, DominatorTree &DT,
                                             PostDominatorTree &PDT)
    : vecInfo(VecInfo),
      layout(VecInfo.getScalarFunction().getParent()->getDataLayout()), LI(LI),
...
bool VectorizationAnalysis::propagateJoinDivergence(const BasicBlock &JoinBlock,
  ...
  if (!vecInfo.inRegion(JoinBlock)) {
    IF_DEBUG_VA
    llvm::errs() << "VA: detected divergent join outside the region in block "
                 << JoinBlock.getName() << "!\n";
    return false;
  }
...
void VectorizationAnalysis::compute(const Function &F) {
  ...
  while (const Instruction *nextI = takeFromWorklist()) {
    ...
    // Compute at least an explicit 'undef' shape for PHINodes to break
    // dependence cycles.
...
static bool AllUniformOrUndefCall(const VectorizationInfo &VecInfo,
                                  const Instruction &I) {
...
bool VectorizationAnalysis::updateShape(const Value &V, VectorShape AT) {
  ...
  IF_DEBUG_VA {
    errs() << "input shapes for " << V << " " << Old << " " << New << "\n";
  }
