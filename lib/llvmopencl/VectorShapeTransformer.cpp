...
pocl::VectorShape GenericTransfer(pocl::VectorShape a) {
  if (!a.isDefined())
    return a;
  return a.isUniform() ? pocl::VectorShape::uni()
                       : pocl::VectorShape::varying();
}
...
pocl::VectorShape GenericTransfer(pocl::VectorShape a,
  ...
  if (a.isDefined() && !a.isUniform())
    return a.isDefined() ? pocl::VectorShape::varying() : VectorShape::undef();
  else
