...
half _cl_log1p(half x0)
{
  ...
  vecmathlib::realpseudovec<half,1> r = log1p(y0);
  return (r)[0];
}
#else
...
half _cl_log1p(half x0)
{
  ...
  vecmathlib::realbuiltinvec<half,1> r = log1p(y0);
  return (r)[0];
}
#endif
...
// Implement log1p by calling vecmathlib
half2 _cl_log1p(half2 x0)
{
  vecmathlib::realvec<half,2> y0 = bitcast<half2,vecmathlib::realvec<half,2> >(x0);
  vecmathlib::realvec<half,2> r = vecmathlib::log1p(y0);
...
half4 _cl_log1p(half4);
half2 _cl_log1p(half2 x0)
{
  half4 y0 = bitcast<half2,half4>(x0);
  half4 r = _cl_log1p(y0);
...
// Implement log1p by calling vecmathlib
half3 _cl_log1p(half3 x0)
{
  vecmathlib::realvec<half,3> y0 = bitcast<half3,vecmathlib::realvec<half,3> >(x0);
  vecmathlib::realvec<half,3> r = vecmathlib::log1p(y0);
...
// Implement log1p by calling vecmathlib
half4 _cl_log1p(half4 x0)
{
  vecmathlib::realvec<half,4> y0 = bitcast<half4,vecmathlib::realvec<half,4> >(x0);
  vecmathlib::realvec<half,4> r = vecmathlib::log1p(y0);
...
// Implement log1p by using a larger vector size
half8 _cl_log1p(half8);
half4 _cl_log1p(half4 x0)
{
  half8 y0 = bitcast<half4,half8>(x0);
  half8 r = _cl_log1p(y0);
...
// Implement log1p by calling vecmathlib
half16 _cl_log1p(half16 x0)
{
  vecmathlib::realvec<half,16> y0 = bitcast<half16,vecmathlib::realvec<half,16> >(x0);
  vecmathlib::realvec<half,16> r = vecmathlib::log1p(y0);
