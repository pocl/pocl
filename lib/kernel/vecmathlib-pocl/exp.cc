...
// Implement exp by calling vecmathlib
half _cl_exp(half x0)
{
  vecmathlib::realvec<half,1> y0 = bitcast<half,vecmathlib::realvec<half,1> >(x0);
  vecmathlib::realvec<half,1> r = vecmathlib::exp(y0);
  return bitcast<vecmathlib::realvec<half,1>,half>((r));
}
#elif ! defined POCL_VECMATHLIB_BUILTIN
// Implement exp by calling libm
half _cl_exp(half x0)
{
  vecmathlib::realpseudovec<half,1> y0 = x0;
  vecmathlib::realpseudovec<half,1> r = exp(y0);
  return (r)[0];
}
#else
// Implement exp by calling builtin
half _cl_exp(half x0)
{
  vecmathlib::realbuiltinvec<half,1> y0 = x0;
  vecmathlib::realbuiltinvec<half,1> r = exp(y0);
  return (r)[0];
}
#endif
...
// Implement exp by using a larger vector size
half4 _cl_exp(half4);
half2 _cl_exp(half2 x0)
{
...
#else
// Implement exp by splitting into a smaller vector size
half _cl_exp(half);
half2 _cl_exp(half2 x0)
{
...
float _cl_exp(float x0)
{
  ...
  vecmathlib::realpseudovec<float,1> r = exp(y0);
  return (r)[0];
}
#else
// Implement exp by calling builtin
float _cl_exp(float x0)
{
  ...
  vecmathlib::realbuiltinvec<float,1> r = exp(y0);
  return (r)[0];
}
#endif
...
// Implement exp by using a larger vector size
float16 _cl_exp(float16);
float8 _cl_exp(float8 x0)
{
  float16 y0 = bitcast<float8,float16>(x0);
  float16 r = _cl_exp(y0);
...
double _cl_exp(double x0)
{
  ...
  vecmathlib::realpseudovec<double,1> r = exp(y0);
  return (r)[0];
}
#else
// Implement exp by calling builtin
double _cl_exp(double x0)
{
  ...
  vecmathlib::realbuiltinvec<double,1> r = exp(y0);
  return (r)[0];
}
#endif
...
double2 _cl_exp(double2 x0)
{
  ...
  double4 r = _cl_exp(y0);
  return bitcast<double4,double2>(r);
}
#else
// Implement exp by splitting into a smaller vector size
double _cl_exp(double);
double2 _cl_exp(double2 x0)
{
...
// Implement exp by using a larger vector size
double8 _cl_exp(double8);
double4 _cl_exp(double4 x0)
{
  double8 y0 = bitcast<double4,double8>(x0);
  double8 r = _cl_exp(y0);
...
// Implement exp by using a larger vector size
double16 _cl_exp(double16);
double8 _cl_exp(double8 x0)
{
  double16 y0 = bitcast<double8,double16>(x0);
  double16 r = _cl_exp(y0);
