...
// Implement cos by calling vecmathlib
half _cl_cos(half x0)
{
  vecmathlib::realvec<half,1> y0 = bitcast<half,vecmathlib::realvec<half,1> >(x0);
  vecmathlib::realvec<half,1> r = vecmathlib::cos(y0);
  return bitcast<vecmathlib::realvec<half,1>,half>((r));
}
#elif ! defined POCL_VECMATHLIB_BUILTIN
// Implement cos by calling libm
half _cl_cos(half x0)
{
  vecmathlib::realpseudovec<half,1> y0 = x0;
  vecmathlib::realpseudovec<half,1> r = cos(y0);
  return (r)[0];
}
#else
// Implement cos by calling builtin
half _cl_cos(half x0)
{
  vecmathlib::realbuiltinvec<half,1> y0 = x0;
  vecmathlib::realbuiltinvec<half,1> r = cos(y0);
  return (r)[0];
}
#endif
...
// Implement cos by using a larger vector size
half4 _cl_cos(half4);
half2 _cl_cos(half2 x0)
{
...
#else
// Implement cos by splitting into a smaller vector size
half _cl_cos(half);
half2 _cl_cos(half2 x0)
{
...
float _cl_cos(float x0)
{
  ...
  vecmathlib::realpseudovec<float,1> r = cos(y0);
  return (r)[0];
}
#else
// Implement cos by calling builtin
float _cl_cos(float x0)
{
  ...
  vecmathlib::realbuiltinvec<float,1> r = cos(y0);
  return (r)[0];
}
#endif
...
// Implement cos by using a larger vector size
float16 _cl_cos(float16);
float8 _cl_cos(float8 x0)
{
  float16 y0 = bitcast<float8,float16>(x0);
  float16 r = _cl_cos(y0);
...
double _cl_cos(double x0)
{
  ...
  vecmathlib::realpseudovec<double,1> r = cos(y0);
  return (r)[0];
}
#else
// Implement cos by calling builtin
double _cl_cos(double x0)
{
  ...
  vecmathlib::realbuiltinvec<double,1> r = cos(y0);
  return (r)[0];
}
#endif
...
double2 _cl_cos(double2 x0)
{
  ...
  double4 r = _cl_cos(y0);
  return bitcast<double4,double2>(r);
}
#else
// Implement cos by splitting into a smaller vector size
double _cl_cos(double);
double2 _cl_cos(double2 x0)
{
...
// Implement cos by using a larger vector size
double8 _cl_cos(double8);
double4 _cl_cos(double4 x0)
{
  double8 y0 = bitcast<double4,double8>(x0);
  double8 r = _cl_cos(y0);
...
// Implement cos by using a larger vector size
double16 _cl_cos(double16);
double8 _cl_cos(double8 x0)
{
  double16 y0 = bitcast<double8,double16>(x0);
  double16 r = _cl_cos(y0);
