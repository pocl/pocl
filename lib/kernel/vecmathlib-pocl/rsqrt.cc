...
half _cl_rsqrt(half x0)
{
  ...
  vecmathlib::realpseudovec<half,1> r = rsqrt(y0);
  return (r)[0];
}
#else
...
half _cl_rsqrt(half x0)
{
  ...
  vecmathlib::realbuiltinvec<half,1> r = rsqrt(y0);
  return (r)[0];
}
#endif
...
// Implement rsqrt by calling vecmathlib
half2 _cl_rsqrt(half2 x0)
{
  vecmathlib::realvec<half,2> y0 = bitcast<half2,vecmathlib::realvec<half,2> >(x0);
  vecmathlib::realvec<half,2> r = vecmathlib::rsqrt(y0);
...
half4 _cl_rsqrt(half4);
half2 _cl_rsqrt(half2 x0)
{
  half4 y0 = bitcast<half2,half4>(x0);
  half4 r = _cl_rsqrt(y0);
  return bitcast<half4,half2>(r);
}
#else
// Implement rsqrt by splitting into a smaller vector size
half _cl_rsqrt(half);
half2 _cl_rsqrt(half2 x0)
{
  pair_half y0 = bitcast<half2,pair_half>(x0);
  pair_half r;
  r.lo = _cl_rsqrt(y0.lo);
  r.hi = _cl_rsqrt(y0.hi);
...
// Implement rsqrt by calling vecmathlib
half3 _cl_rsqrt(half3 x0)
{
  vecmathlib::realvec<half,3> y0 = bitcast<half3,vecmathlib::realvec<half,3> >(x0);
  vecmathlib::realvec<half,3> r = vecmathlib::rsqrt(y0);
...
// Implement rsqrt by calling vecmathlib
half4 _cl_rsqrt(half4 x0)
{
  vecmathlib::realvec<half,4> y0 = bitcast<half4,vecmathlib::realvec<half,4> >(x0);
  vecmathlib::realvec<half,4> r = vecmathlib::rsqrt(y0);
...
// Implement rsqrt by using a larger vector size
half8 _cl_rsqrt(half8);
half4 _cl_rsqrt(half4 x0)
{
  half8 y0 = bitcast<half4,half8>(x0);
  half8 r = _cl_rsqrt(y0);
...
// Implement rsqrt by calling vecmathlib
half16 _cl_rsqrt(half16 x0)
{
  vecmathlib::realvec<half,16> y0 = bitcast<half16,vecmathlib::realvec<half,16> >(x0);
  vecmathlib::realvec<half,16> r = vecmathlib::rsqrt(y0);
...
float2 _cl_rsqrt(float2 x0)
{
  ...
  float4 r = _cl_rsqrt(y0);
  return bitcast<float4,float2>(r);
}
#else
// Implement rsqrt by splitting into a smaller vector size
float _cl_rsqrt(float);
float2 _cl_rsqrt(float2 x0)
{
...
float16 _cl_rsqrt(float16 x0)
{
  ...
  vecmathlib::realvec<float,16> r = vecmathlib::rsqrt(y0);
...
double2 _cl_rsqrt(double2 x0)
{
  ...
  double4 r = _cl_rsqrt(y0);
  return bitcast<double4,double2>(r);
}
#else
// Implement rsqrt by splitting into a smaller vector size
double _cl_rsqrt(double);
double2 _cl_rsqrt(double2 x0)
{
...
double16 _cl_rsqrt(double16 x0)
{
  ...
  vecmathlib::realvec<double,16> r = vecmathlib::rsqrt(y0);
