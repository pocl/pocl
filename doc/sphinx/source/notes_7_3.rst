...
* Vectorized math libraries (libmvec, SLEEF) are now filtered per function:
  ...
  width 1, double `exp` at one argument and `log1p` in both precisions
  (glibc libmvec), and double `pow` near the overflow threshold (SLEEF
  older than 3.8). `POCL_VECMATH_DENY` / `POCL_VECMATH_ALLOW` override
