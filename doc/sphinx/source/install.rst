...
- ``-DENABLE_HOST_CPU_VECTORIZE_{LIBMVEC,SLEEF,SVML}=ON/OFF``
  Let the CPU device's kernel compiler call a vector math library (glibc
  libmvec, SLEEF's libmvec-ABI build, or Intel SVML) for the OpenCL math
  builtins whenever the work-item loop vectorizes. Functions measured to
  exceed the OpenCL ULP bounds in a given library are kept scalar by a
  per-library deny list (``pocl_vecmath_deny.h``, generated by CMake from
  ``lib/CL/pocl_vecmath_deny.h.in``). libmvec and SLEEF default to ON when found, except together with
  ``ENABLE_CONFORMANCE``, where they default to OFF and are opt-in. SVML has
  no deny list and cannot be combined with ``ENABLE_CONFORMANCE``. On x86-64
  with glibc 2.35 or newer the whole libmvec function set is used, not only
  the six functions in LLVM's own table; the AVX-512 variants are used only
  on hosts that have AVX-512.
