/*
  ...
  gengetopt --input=cmdline.input --include-getopt
...
const char *gengetopt_args_info_purpose = "remote OpenCLserver";

const char *gengetopt_args_info_usage = "Usage: pocld [OPTION]...";
 ...
 */
static void
exchange (char **argv, struct custom_getopt_data *d)
{
...
static void
custom_getopt_initialize (struct custom_getopt_data *d)
{
  /*
...
static int
shuffle_argv (int argc, char *const *argv, const struct option *longopts,
              struct custom_getopt_data *d)
{
   ...
   * punctuation.
   */
  d->nextchar = (argv[d->custom_optind] + 1
                 + (longopts != NULL && argv[d->custom_optind][1] == '-'));
  return 0;
}
 ...
 */
static int
check_long_opt (int argc, char *const *argv, const char *optstring,
                const struct option *longopts, int *longind, int print_errors,
                struct custom_getopt_data *d)
{
  char *nameend;
                  ...
                  if (argv[d->custom_optind - 1][1] == '-')
                  ...
                  else
                    {
                      ...
                      fprintf (stderr,
                               ...
                               argv[0], argv[d->custom_optind - 1][0],
                               pfound->name);
                    }
                }
              d->nextchar += strlen (d->nextchar);
...
static int
check_short_opt (int argc, char *const *argv, const char *optstring,
 ...
 * that wants an optional arg; if there is text in the current ARGV-element, it
 * is returned in `custom_optarg', otherwise `custom_optarg' is set to zero.
 *
...
static int
getopt_internal_r (int argc, char *const *argv, const char *optstring,
                   ...
                   struct custom_getopt_data *d)
{
  ...
  if (longopts && (argv[d->custom_optind][1] == '-'))
    return check_long_opt (argc, argv, optstring, longopts, longind,
                           print_errors, d);
  return check_short_opt (argc, argv, optstring, print_errors, d);
}

static int
custom_getopt_internal (int argc, char *const *argv, const char *optstring,
                        const struct option *longopts, int *longind)
{
  int result;
  ...
  result = getopt_internal_r (argc, argv, optstring, longopts, longind, &d);
...
static int
custom_getopt_long (int argc, char *const *argv, const char *options,
 ...
 */
static int
update_arg (void *field, char **orig_field, unsigned int *field_given,
      ...
      switch (c)
        ...
        case 'a': /* Listen address.  */
          ...
          if (update_arg ((void *)&(args_info->address_arg),
                          ...
                          &(args_info->address_given),
                          &(local_args_info.address_given), optarg, 0, 0,
                          ARG_STRING, check_ambiguity, override, 0, 0,
                          "address", 'a', additional_error))
            goto failure;
        ...
        case 'p': /* Listen port.  */
          ...
          break;
        case 'v': /* Program log category filter.  */

          if (update_arg ((void *)&(args_info->log_filter_arg),
                          &(args_info->log_filter_orig),
                          &(args_info->log_filter_given),
