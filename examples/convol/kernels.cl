...
void
atomicAdd (volatile __global float *source, const float operand)
{
  union
  {
    uint intVal;
...
size_t
getNHWC (ShapeNHWC s, cl_uint32_t n, cl_uint32_t h, cl_uint32_t w,
         cl_uint32_t c)
{
...
size_t
getNCHW (ShapeNCHW s, cl_uint32_t n, cl_uint32_t c, cl_uint32_t h,
         cl_uint32_t w)
{
...
cl_int32_t
scale_i32i8 (cl_int32_t input, cl_int32_t pre, cl_int32_t post,
...
cl_int8_t
quantize (float input, float scale, cl_int32_t offset)
{
...
cl_int32_t
quantize_i32 (float input, float scale, cl_int32_t offset)
{
  float result = input / scale + offset;
...
float
dequantize (cl_int8_t input, float scale, cl_int32_t offset)
{
  return scale * (input - offset);
}

__kernel void
oclbatchedreduceaddK (__global float *dest, __global float *batch,
                      ...
                      cl_uint32_t numSlices, cl_uint32_t axisSliceSize)
{
  ...
  //
  // For example, suppose the input shape is {3, 4, 5} and the reduce axis
  // is 1. Then, the output shape is {3, 5}. In this case, batchSliceSizes is
  // {4 * 5 = 20, 1} (axis 1 is missing) and destSliceSizes is {5, 1}. The
  // global workspace this kernel was launched with has dimensions {3, 5} (one
  // for each output element). A kernel with IDs {i, j} will add together
  // elements {i, 0..4, j} and store the result in element {i, j}, so (i * 20 +
  // j * 1) will be a component of every offset it uses to access batch, and (i
  // * 5 + j
...
__kernel void
batchedaddK (__global float *dest, __global float *batch,
...
__kernel void
batchedaddW (__global void *mem, cl_uint32_t dest, cl_uint32_t batch,
             cl_uint32_t slice, cl_uint32_t numSlice, cl_uint32_t sliceSize)
{
  batchedaddK (&mem[dest], &mem[batch], &mem[slice], numSlice, sliceSize);
}

__kernel void
batchedadd_i8K (__global cl_int8_t *dest, __global cl_int8_t *batch,
  ...
  for (size_t n = 0; n < numSlice; n++)
    {
      ...
      cl_int32_t x
          = scale_i32i8 (batchVal, batchPre, batchPost, batchScale, 0);
      cl_int32_t y
          = scale_i32i8 (sliceVal, slicePre, slicePost, sliceScale, 0);
      dest[n * sliceSize + s] = clip (x + y + destOffset);
    }
}

__kernel void
batchedadd_i8W (__global void *mem, cl_uint32_t dest, cl_uint32_t batch,
                cl_uint32_t slice, cl_uint32_t numSlice, cl_uint32_t sliceSize,
...
__kernel void
batchedadd_i8K_32 (__global cl_int8_t *dest, __global cl_int8_t *batch,
                   __global cl_int32_t *slice, cl_uint32_t numSlice,
                   cl_uint32_t sliceSize, cl_int32_t destOffset,
                   cl_int32_t batchOffset, cl_int32_t sliceOffset,
                   cl_int32_t batchPre, cl_int32_t batchPost,
                   cl_int32_t batchScale, cl_int32_t slicePre,
...
__kernel void
batchedadd_i8W_32 (__global void *mem, cl_uint32_t dest, cl_uint32_t batch,
                   cl_uint32_t slice, cl_uint32_t numSlice,
                   cl_uint32_t sliceSize, cl_int32_t destOffset,
                   QuantizationTransform32To8 batchScaleParams,
                   QuantizationTransform32To8 sliceScaleParams)
{
...
__kernel void
matmul_tiled (__global void *mem, cl_uint32_t C_off, cl_uint32_t A_off,
              cl_uint32_t B_off, ShapeNHWC ddim, ShapeNHWC ldim,
              ShapeNHWC rdim)
{
...
__kernel void
matmul_tiled_i8 (__global void *mem, cl_uint32_t C_off, cl_uint32_t A_off,
                 cl_uint32_t B_off, ShapeNHWC ddim, ShapeNHWC ldim,
                 ShapeNHWC rdim, cl_int32_t aOffset, cl_int32_t bOffset,
                 cl_int32_t cOffset,
...
__kernel void
matmulK (__global float *dest, __global float *lhs, __global float *rhs,
...
__kernel void
matmulW (__global void *mem, cl_uint32_t dest, cl_uint32_t lhs,
         cl_uint32_t rhs, ShapeNHWC ddim, ShapeNHWC ldim, ShapeNHWC rdim)
{
  matmulK (&mem[dest], &mem[lhs], &mem[rhs], ddim, ldim, rdim);
}

__kernel void
matmul_i8K (__global cl_int8_t *dest, __global cl_int8_t *lhs,
            __global cl_int8_t *rhs, ShapeNHWC ddim, ShapeNHWC ldim,
            ShapeNHWC rdim, cl_int32_t lhsOffset, cl_int32_t rhsOffset,
            cl_int32_t destOffset, cl_int32_t destPre, cl_int32_t destPost,
...
__kernel void
matmul_i8W (__global void *mem, cl_uint32_t dest, cl_uint32_t lhs,
            cl_uint32_t rhs, ShapeNHWC ddim, ShapeNHWC ldim, ShapeNHWC rdim,
            cl_int32_t lhsOffset, cl_int32_t rhsOffset, cl_int32_t destOffset,
            QuantizationTransform32To8 destScaleParams)
{
...
__kernel void
softmaxK (__global float *dest, __global float *src, __global float *e_cache,
          cl_uint32_t sliceSize)
{
...
__kernel void
softmaxW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
          cl_uint32_t sliceSize)
{
...
__kernel void
softmaxgradK (__global float *inG, __global float *outW,
...
__kernel void
softmaxgradW (__global void *mem, cl_uint32_t origDest, cl_uint32_t origSrc,
              cl_uint32_t selected, cl_uint32_t srcGrad, cl_uint32_t sliceSize)
{
  softmaxgradK (&mem[srcGrad], &mem[origDest], &mem[selected], sliceSize);
}

__kernel void
convolutionK (__global float *dest, __global float *src,
              __global float *filter, __global float *bias,
              ShapeHW kernelSizes, ShapeHW strides, PaddingTLBR pads,
...
__kernel void
convolutionW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
  ...
  for (size_t n = 0; n < idim.n; n++)
    {
      ...
      dest[getNHWC (odim, n, ax, ay, d)] = clip (
          round ((float)(sum) * (matMulScale / destScale) + destOffset));
    }
}
...
{
  convolution_i8K (&mem[dest], &mem[src], &mem[filter], &mem[bias],
                   kernelSizes, strides, destOffset, destScale, srcOffset,
                   srcScale, filterOffset, filterScale, biasOffset, biasScale,
                   pads, group, dilation, odim, idim, filterDim);
}

__kernel void
convolutiongradK (const __global float *inW, const __global float *filterW,
                  const __global float *outG, __global float *inG,
                  __global float *filterG, __global float *biasG,
                  ShapeHW kernelSizes, ShapeHW strides, PaddingTLBR pads,
                  cl_uint32_t group, cl_uint32_t dilation, ShapeNHWC inWdims,
              ...
              for (size_t fd = 0; fd < inCperG; fd++)
                {
                  ...
                  atomicAdd (&inG[getNHWC (inWdims, n, (size_t)ox, (size_t)oy,
                                           fd + inChannelOffset)],
                             filterW[getNHWC (filterGdims, d, fx, fy, fd)]
                                 * grad);
                }
            }
        }
...
__kernel void
convolutiongradW (__global void *mem, cl_uint32_t src, cl_uint32_t filter,
                  cl_uint32_t destGrad, cl_uint32_t srcGrad,
                  cl_uint32_t filterGrad, cl_uint32_t biasGrad,
                  ShapeHW kernelSizes, ShapeHW strides, PaddingTLBR pads,
                  cl_uint32_t group, cl_uint32_t dilation, ShapeNHWC srcDim,
                  ShapeNHWC destGradDim, ShapeNHWC filterGradDim)
{
  convolutiongradK (&mem[src], &mem[filter], &mem[destGrad], &mem[srcGrad],
                    &mem[filterGrad], &mem[biasGrad], kernelSizes, strides,
                    pads, group, dilation, srcDim, destGradDim, filterGradDim);
}

__kernel void
maxpoolK (__global float *dest, __global float *src, cl_uint32_t kernelSize,
          cl_uint32_t stride, PaddingTLBR pads, ShapeNHWC odim, ShapeNHWC idim)
{
  size_t ax = get_global_id (0);
  size_t ay = get_global_id (1);
  size_t d = get_global_id (2);
...
__kernel void
maxpoolW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
...
#define DEFINE_OPENCL_MAXPOOL_KERNEL(name, type)                              \
  ...
  {                                                                           \
    ...
    for (size_t n = 0; n < idim.n; n++)                                       \
      {                                                                       \
        type maxVal = MIN_##type;                                             \
        /* For each element in the convolution-filter: */                     \
        for (size_t ox = x; ox < (x + kernelSize); ox++)                      \
          {                                                                   \
            for (size_t oy = y; oy < (y + kernelSize); oy++)                  \
              {                                                               \
                /* Ignore index access below zero (this is due to padding).   \
                 */                                                           \
                if (ox < 0 || oy < 0 || ox >= (ssize_t)idim.h                 \
                ...
                type val = src[getNCHW (idim, n, d, ox, oy)];                 \
                if (val >= maxVal)                                            \
                  {                                                           \
                    maxVal = val;                                             \
                  }                                                           \
...
__kernel void
maxpoolwithargmaxK (__global float *dest, __global float *src,
          ...
          for (size_t fy = 0; fy < kernelSize; fy++)
            {
              ...
              const size_t flatIndex
                  = getNHWC (idim, n, (size_t)ox, (size_t)oy, d);
...
__kernel void
maxpoolwithargmaxW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
                    cl_uint32_t argmax, cl_uint32_t kernelSize,
                    cl_uint32_t stride, PaddingTLBR pads, ShapeNHWC odim,
                    ShapeNHWC idim)
{
...
__kernel void
maxpoolwithargmaxgradK (__global float *dest, __global dim_t *argmax,
...
__kernel void
maxpoolwithargmaxgradW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
...
__kernel void
avgpoolK (__global float *dest, __global float *src, cl_uint32_t kernelSize,
          cl_uint32_t stride, PaddingTLBR pads, ShapeNHWC odim, ShapeNHWC idim)
{
  size_t ax = get_global_id (0);
  size_t ay = get_global_id (1);
...
__kernel void
avgpoolW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
          cl_uint32_t kernelSize, cl_uint32_t stride, PaddingTLBR pads,
          ShapeNHWC odim, ShapeNHWC idim)
{
...
__kernel void
oclavgpoolK (__global float *dest, __global float *src, cl_uint32_t kernelSize,
             cl_uint32_t stride, PaddingTLBR pads, ShapeNCHW odim,
             ShapeNCHW idim)
{
  size_t ax = get_global_id (0);
  size_t ay = get_global_id (1);
...
__kernel void
oclavgpoolW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
             cl_uint32_t kernelSize, cl_uint32_t stride, PaddingTLBR pads,
             ShapeNCHW odim, ShapeNCHW idim)
{
...
__kernel void
oclavgpool_i8K (__global cl_int8_t *dest, __global cl_int8_t *src,
                cl_uint32_t kernelSize, cl_uint32_t stride, PaddingTLBR pads,
  ...
  for (size_t n = 0; n < idim.n; n++)
    {
      ...
      // All dest scale params are already divided by kernel*kernel(filter
      // area), we don't need to divide the sumVal by filter area here.
      dest[getNCHW (odim, n, d, ax, ay)] = clip (
          scale_i32i8 (sumVal, destPre, destPost, destScale, destOffset));
    }
}

__kernel void
oclavgpool_i8W (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
                cl_uint32_t kernelSize, cl_uint32_t stride, PaddingTLBR pads,
                ShapeNCHW odim, ShapeNCHW idim, cl_int32_t srcOffset,
                QuantizationTransform32To8 destScaleParams)
{
...
#define DEFINE_OPENCL_TRANSPOSE_KERNEL(name, type)                            \
  ...
  __kernel void name##W (__global char *mem, cl_uint32_t de, cl_uint32_t sr,  \
                         ShapeNHWC odim, ShapeNHWC idim, ShapeNHWC shuffle)   \
  {                                                                           \
    size_t res[4];                                                            \
    size_t d0 = get_global_id (0);                                            \
    size_t d1 = get_global_id (1);                                            \
    __global char *memde = mem + de;                                          \
    __global char *memsr = mem + sr;                                          \
    __global type *dest = (__global type *)memde;                             \
    __global type *src = (__global type *)memsr;                              \
    res[0] = d0;                                                              \
    res[1] = d1;                                                              \
    for (size_t d2 = 0; d2 < idim.w; d2++)                                    \
      {                                                                       \
...
#define DEFINE_OPENCL_INSERT_TENSOR_KERNEL(name, type)                        \
  ...
  __kernel void name##W (                                                     \
      __global void *mem, cl_uint32_t dest, cl_uint32_t src, ShapeNHWC odim,  \
      ShapeNHWC idim, ShapeNHWC offset, cl_uint32_t count, cl_uint32_t axis)  \
  {                                                                           \
...
void
memcpy_float (__global float *dest, const __global float *src, int len)
{
  for (int i = 0; i < len; i++)
    {
      dest[i] = src[i];
...
__kernel void
gatherK (__global float *dest, __global const float *src,
...
__kernel void
gatherW (__global void *mem, cl_uint32_t dest, cl_uint32_t src,
         cl_uint32_t indices, cl_uint32_t numIndices, cl_uint32_t sliceSize,
         cl_uint32_t numSamples, cl_uint32_t destSampleSize,
...
__kernel void
scatterdataK (__global float *data, __global dim_t *indices,
              __global const float *slices, cl_uint32_t sliceSize)
{
  int idx = get_global_id (0);
...
__kernel void
scatterdataW (__global void *mem, cl_uint32_t data, cl_uint32_t indices,
              cl_uint32_t slices, cl_uint32_t sliceSize)
{
                           ...
                           __global cl_int32_t *lengths, cl_uint32_t sliceSize)
{
  ...
  // Add together the segment lengths of the segments being processed by
  // kernels with global ID 0 .. idx - 1. This is the index into the indices
  // buffer at which *this* kernel should start reading indices with which to
  // index into the data buffer and weights that the data should be multiplied
  // by.
  cl_int32_t curIdx = 0;
  ...
  for (cl_int32_t j = 0; j < lengths[idx]; ++j)
    {
      ...
      // Read an entire slice of data at index dataIdx, multiply it by the
      // weight and store it into dest at index idx (same as the segment
      // number).
...
__kernel void
sparselengthsweightedsumW (__global void *mem, cl_uint32_t dest,
...
__kernel void
sparselengthsweightedsumgradK (__global float *destGrad,
                               __global float *dataGrad,
                               __global float *weightsGrad,
                               __global float *data, __global float *weights,
                               __global dim_t *indices,
      ...
      for (cl_int32_t j = 0; j < lengths[i]; ++j, ++curIdx)
        {
          ...
          // corresponding slice of dest during the forward pass) and store it
          // into dataGrad at index dataGradIdx. In the same loop, accumulate
          // into each weight gradient the reduced sum of the elementwise
          // product of the result slice that the corresponding weight produced
          // and the input slice that the weight was multiplied with.
          for (cl_uint32_t k = 0; k < sliceSize; ++k)
            {
              dataGrad[dataGradIdx * sliceSize + k]
                  += weight * destGrad[i * sliceSize + k];
              weightGrad += destGrad[i * sliceSize + k]
                            * data[dataGradIdx * sliceSize + k];
            }
