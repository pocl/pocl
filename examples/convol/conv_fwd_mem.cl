...
__kernel __attribute__ ((reqd_work_group_size (workgroup_size_0,
                                               workgroup_size_1, 1)))
...
#pragma unroll
        for (int_tp wn = 0; wn < WPTN / VWN; ++wn)
          {
            VEC_4_0 (Creg[wm][wn]) = 0.0f;
            VEC_4_1 (Creg[wm][wn]) = 0.0f;
            VEC_4_2 (Creg[wm][wn]) = 0.0f;
            VEC_4_3 (Creg[wm][wn]) = 0.0f;
          }
      }
    {
// Loop over all tiles.
                ...
                if ((offN + col) < NN && tiledIndex < KK)
                  {
                    ...
                    // Compute d_iter, final tiledIndex becomes input feature
                    // map ID. Scale d_iter by the dilation factor.
                    ...
                    if (skip_range_check || in_range)
                      {
                        // tiledIndex now holds the memory offset for the input
                        // image.
