  ...
  */  }; 
   uint16_t header = packet->header;
    if (header & (1 << AQL_PACKET_BARRIER_AND))
    {
...
#ifdef BASE_ADDRESS
      __buffer__ uint32_t* control_region = (__buffer__ uint32_t*)BASE_ADDRESS;
      ...
      __buffer__ struct CommandMetadata* cmd_meta = packet->completion_signal_low;
      ...
      //cmd_meta->start_timestamp_h = cc_h + sc_h;
        ...
        __buffer__ uint32_t* kernarg_ptr = (__buffer__ uint32_t*)(packet->kernarg_address_low);
