...
generatebits --data-start=param,${DMEM_OFFSET} -d -w 4 -e tta_core -p standalone.tpef ../firmwares/${1}.adf
COMPL_SIGNAL_ADDR=$(echo "mach ../firmwares/${1}.adf;prog standalone.tpef;symbol_address __completion_signal" |\
    ttasim | head -n1)

COMPL_SIGNAL_ADDR=$((COMPL_SIGNAL_ADDR - DMEM_OFFSET))

pushd rtl_${1} > /dev/null
...
bash ghdl_simulate.sh -v93c -r 2000000 -n tta_almaif_tb -g "imem_image=../standalone.img"\
    ...
    -g "dmem_dump_path=../rtlsim_raw_output.txt" > /dev/null
popd > /dev/null

OUTPUT_BUFFER_SYMBOL=$(tail -n1 standalone_0_ttasim |cut -d' ' -f6 |cut -d';' -f1)

OUTPUT_BUFFER_ADDR=$(echo "mach ../firmwares/${1}.adf;prog standalone.tpef;symbol_address ${OUTPUT_BUFFER_SYMBOL}" |\
    ttasim | head -n1)
